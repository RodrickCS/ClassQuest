...
const add_points = async (req, res) => {
  try {
    let ponto =
      await prisma.$queryRaw`UPDATE pontos SET qtd = (SELECT qtd FROM pontos WHERE id_aluno = ${req.params.id_aluno} AND id_turma = ${req.params.id_turma} LIMIT 1) + ${req.body.qtd} WHERE id_aluno = ${req.params.id_aluno} AND id_turma = ${req.params.id_turma}`;
